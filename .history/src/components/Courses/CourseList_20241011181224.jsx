...
const CourseList = ({ isAdmin = false, onUpdateStatus, onDelete }) => {
  ...
  const fetchUserCourses = async () => {
    try {
      const user =  storage.get('access_token');
      console.log(user);
      const response = await api.get('/courses/user/'+ user._id);
      setCourses(response.data);
    } catch (error) {
      console.error('Erreur lors de la récupération des courses:', error);
    }
  ...
  const handleUpdateStatus = async (id, status) => {
    console.log(id, status);
      const response = await api.patch(`/courses/${id}/status`, {status: status});
      fetchUserCourses();
            ...
            <div className="col-md-6 mb-4" key={course._id}>
              <div className="card shadow">
                <div className="card-header bg-dark text-white">
                  <h5 className="card-title">Course #{course._id}</h5>
                </div>
